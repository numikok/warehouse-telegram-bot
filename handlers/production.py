...
@router.message(ProductionStates.waiting_for_film_thickness)
async def process_film_thickness(message: Message, state: FSMContext):
    if message.text == "◀️ Назад":
    ...
    try:
        ...
        try:
            ...
            if not glue or glue.quantity < quantity:
                await message.answer(
                    f"Недостаточно клея. Доступно: {glue.quantity if glue else 0} шт.",
                    reply_markup=get_menu_keyboard(MenuState.PRODUCTION_MAIN)
                )
                return
            ...
            await message.answer(
                f"✅ Учтено {quantity} шт. бракованного клея\n"
                f"Общий остаток: {glue.quantity} шт.",
                reply_markup=get_menu_keyboard(MenuState.PRODUCTION_MAIN)
            )
            
            logging.info(f"Создана запись об операции с бракованным клеем: {quantity} шт.")
